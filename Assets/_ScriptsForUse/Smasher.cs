...
public class Smasher 
{
    ...
    public ShelfLife shelfLifeValue; // Smasher's Default Health Bar.
    [SerializeField] private float speed; // Smasher's Default Walking Speed.
    [SerializeField] private float runningSpeed; // Smasher's Default Running Speed.
    [SerializeField] private float crouchMoveSpeed; // Smasher's Default Crouching Move Speed.
    [SerializeField] private int fireRate; // Smasher's Default Rate Of Fire based on his Training and Experience.
    [SerializeField] private float morale; // Smasher's starting Default Level of Morale.
    [SerializeField] private int size; // Smasher's current Size. (Determines Smasher's permanent increase in starting health and morale from a Minimum to a Maximum Size.)
    [SerializeField] private int strength; // Smasher's current strength. (Strength determines ability to wield weapons.)
    ...
    public Smasher(float speed, int shelflifevalue)
    {
        this.speed = speed;
        shelfLifeValue = new ShelfLife(shelflifevalue); // this value is for the different enemies spawning.
    }
}
